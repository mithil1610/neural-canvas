...
class AiService {
  ...
  final ValueNotifier<List<ChatMessage>> chatHistory = ValueNotifier([
    const ChatMessage(
      text:
          'Hello! I am your Digital Memory Assistant. How can I help you explore your canvas today?',
      isAssistant: true,
    ),
  ]);
  ...
  Future<String> sendSystemContext(
    String contextText, {
    String? mediaPath,
    String? mediaType,
  }) async {
    ...
    final result = await _callBackendFunction(
      contextText,
      mediaPath: mediaPath,
      mediaType: mediaType,
    );
  ...
  Future<String> _callBackendFunction(
    String prompt, {
    String? mediaPath,
    String? mediaType,
  }) async {
    ...
    _addMessage(
      const ChatMessage(text: '', isAssistant: true, isStreaming: true),
    );
    ...
    try {
      ...
      final Map<String, dynamic> payload = {'prompt': prompt};
