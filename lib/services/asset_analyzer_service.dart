...
class AssetAnalyzerService {
  ...
  static Future<void> analyzeIngestedAsset(
    String docId,
    String fileUrl,
    String fileType,
  ) async {
    ...
    try {
      ...
      String systemPrompt =
          '''You are an expert data extraction engine for a personal Second Brain app.
...
''';
      ...
      if (fileType == 'pdf') {
        ...
        systemPrompt +=
            'Extract all textual data via deep OCR. Identify core concepts, critical names, data points, entities, and provide an exhaustive analytical summary.';
      } else if (['mp3', 'm4a', 'wav'].contains(fileType)) {
        mimeType = 'audio/mp3';
        systemPrompt +=
            'Listen to this audio track carefully. Transcribe the spoken text entirely, analyze the conversational tone, summarize the core topics discussed, and output key takeaways.';
      } else if (['mp4', 'mov'].contains(fileType)) {
        mimeType = 'video/mp4';
        systemPrompt +=
            'Watch this video clip sequentially. Detect key scenes, summarize visual actions, transcribe background audio tracks, and create a timeline summary of what occurs.';
      } else {
        mimeType = 'image/jpeg';
        systemPrompt +=
            'Perform an image analysis. Read any overlay text via OCR, detect visible objects, actions, human expressions, emotional mood, and summarize the overall context.';
      }

      systemPrompt +=
          ' Format your response strictly as JSON: { "title": "...", "summary": "...", "category": "..." }. For the category field, strictly choose the single best fit from these exact 6 options: [health, people, photos, work, documents, events].';

      final response = await http.get(Uri.parse(fileUrl));
      final bytes = response.bodyBytes;

      final content = [
        Content.multi([DataPart(mimeType, bytes), TextPart(systemPrompt)]),
      ];
      ...
      String categoryToSave = "documents";
      ...
      try {
        ...
        if (parsed['category'] != null) {
          categoryToSave = parsed['category'].toString().toLowerCase();
        }
      } catch (e) {
  ...
  static Future<void> _updateSummaryAndTitle(
    String docId,
    String summary,
    String title, [
